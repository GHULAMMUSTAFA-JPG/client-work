                        ...
                        <div className="dropdown ms-2">
                            <a className="btn bg-transparent dropdown-toggle border-0" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                <div className="d-flex align-items-center">
                                   {userProfile?.Profile_Image ? <Image src={userProfile?.Profile_Image} alt="user" width={32} height={32} className="user-img" /> : <h6>{userProfile?.Name ? userProfile?.Name?.slice(0,2) : user?.email ? user?.email?.slice(0,2) :  "NA" }</h6>}
