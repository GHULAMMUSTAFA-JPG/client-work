...
const ThankYouContent = () => {
  ...
  useEffect(() => {
    if (checkoutSession) {
      console.log("checkoutSession found", checkoutSession);
      const updatedUserString = localStorage.getItem("user");
  ...
  }, [checkoutSession, router, setUser]);

  // If no checkout session, show error or redirect
  if (!checkoutSession) {
    console.log("checkoutSession not found", checkoutSession);
    return (
      <div className="container">
        <div className="card">
          <h1 style={{ color: "#dc3545" }}>Invalid Session</h1>
  ...
  // Return null if checkout session exists, as the user will be redirected
  return null;
};
